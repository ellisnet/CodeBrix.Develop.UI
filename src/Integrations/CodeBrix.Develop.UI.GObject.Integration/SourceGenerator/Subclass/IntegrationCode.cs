    ...
    private static string ToCode(ImmutableArray<SubclassData> subclassData)
    {
        return $$"""
                 ...
                 internal static class Integration
                 {
                    ...
                    {{GeneratedCodeAttribute.Render()}}
                    private static void Register<T>() where T : CodeBrix.Develop.UI.GObject.GTypeProvider
                    {
                        try
                        {
                            _ = T.GetGType();
                        ...
                        {
                            throw new System.Exception($"Could not register type {typeof(T).FullName}. Ensure that you initialize depending modules by calling their module initializer first. E.g. call 'CodeBrix.Develop.UI.Gtk.Module.Initialize()' before calling 'GirCore.Integration.Initialize()' if your subclass depends on Gtk.", ex);
                        }
                    }
                 }
                 """;
